function [Total_Cost] = cost_function(number,diameters,power,receivers,transmitters,year)
%The cost_function outputs the total cost for to build antenna arrays based
%on the number, diameter, and power. This cost includes both the
%transmitting and receiving arrays

%Inputs: number = number of elements for a particular diameter as a vector
...
%year = Year of building to account for inflation, a double
...
%depth and width
Trenching = trenching_scale*trenching_per_m*(sum(number,'all')/3)^1.6/(spacing_by_diameter*max(diameters)*1000)*3;
...
Receivec = number.*receivers*cost_receiver; %$
...
Transmitterc = number.*transmitters.*(0.0227*(power/1000)+1.1523)*1E6; %$
...
Concrete = number.*(1E4*(0.0706*diameters.^2+2.2827*diameters-4.4419)); %$
...
Antenna = number.*(254283*exp(diameters.*0.1555)); %$
...
%% Total Cost
cost = sum(Antenna)+sum(Transmitterc)+sum(Concrete)+sum(Receivec)+Trenching+Misc_costs;
...
Total_Cost = cost*(1+interest/100)^(year-2023);
%C is the cost of the total antenna array accounting for compound
